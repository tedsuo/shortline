...
short.load_receivers(function(){
  server.listen(config.port);
  console.log("Shortline (mode: "+environment+") started listening on TCP/" + config.port);
});
...
server.post('/:receiver_name/:path_name', function(req, res){
  if(!_.include(config.trusted_ips, req.client.remoteAddress)){
    res.send("You are not authorized to push to Shortline.");
    console.log('An unauthorized client tried to push to shortline from ip ' + req.client.remoteAddress);
    return;
  }
