...
Job.prototype.createJob = function(callback, override){
  ...
  this.getIndex(function(err, key_id){
    ...
    client.sadd(kp+'_receivers:'+j.receiver_id+':jobs', key_id, function(err){
      if(err) return j.saveError(callback, err);
...
Job.prototype.updateReceiverLists = function(status, callback){
  ...
  if(j.status != "overflow" && status == "overflow"){
    updates.push(function(done){
      client.rpush(kp+'_receivers:'+j.receiver_id+':jobs_overflow', j._id, function(err, result){
    ...
    updates.push(function(done){
      client.lrem(kp+'_receivers:'+j.receiver_id+':jobs_overflow', 1, j._id, function(err, result){
        if(err) return callback(err);
        done();
      });
    });
  ...
  if(j.status != "queued" && status == "queued"){
    updates.push(function(done){
      client.rpush(kp+'_receivers:'+j.receiver_id+':jobs_queued', j._id, function(err, result){
        if(err) return callback(err);
        done();
      });
    });
