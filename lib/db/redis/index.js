...
exports.update_path = function(receiver_name, path_name, options, callback){
  exports.find_receiver_id_by_name(receiver_name, function(err, receiver_id){
    client.hget(kp+'_receivers:'+receiver_id+':paths', path_name, function(err, path_id){
      client.hmset(build_hmset_args(options, kp+'_paths:'+path_id), function(err, result){
        ...
        if(options.name && options.name != path_name){
          client.hdel(kp+'_receiver:'+receiver_id+':paths', path_name, function(err){
            if(err) return callback(err);
            client.hset(kp+'_receiver:'+receiver_id+':paths', options.name, callback);
          });
        }
        return callback(null, result);
      });
...
exports.remove_receiver = function(receiver_name, callback){
  exports.find_receiver_id_by_name(receiver_name, function(err, receiver_id){
    client.hdel(kp+'_receivers_name_id_map', receiver_name, function(err){
      remove_receivers_by_id(receiver_id, callback);
    });
  });
};
...
exports.remove_all = function(callback){
  client.hgetall(kp+'_receivers_name_id_map', err(callback, function(receiver_ids){
    client.del(kp+'_receivers_name_id_map', err(callback, function(){
      var receiver_deletes = [];
      _.each(receiver_ids, function(receiver_id){
        receiver_deletes.push(function(done){
          remove_receiver_by_id(receiver_id, err(callback, function(){
            client.smembers(kp+'_receivers:'+receiver_id+':jobs', err(callback, function(job_ids){
              client.del(kp+'_receivers:'+receiver_id+':jobs', err(callback, function(){
                client.del(kp+'_receivers:'+receiver_id+':jobs_overflow', err(callback, function(){
                  var job_deletes = [];
                  _.each(job_ids, function(job_id){
                    job_deletes.push(function(done){
                      client.del(kp+'_jobs:'+job_id, done);
                    });
                  });
                  async.parallel(job_deletes, done);
                }));
              }));
            }));
          }));
      ...
      async.parallel(receiver_deletes, err(callback, function(){
        client.del(kp+'_jobs_key', err(callback, function(){
          client.del(kp+'_receivers_key', err(callback, function(){
            client.del(kp+'_paths_key', err(callback, function(){
              callback();
            }));
          }));
        }));
...
exports.find_jobs_by_receiver_id = function(receiver_id, options, callback){
  if(options.statuses){
    if(_.include(options.statuses "queued")){
    } else {
    }
    function post_queued_lookup(){
    }
  } else {
    client.smembers(kp+'_receiver:'+receiver_id+':jobs', err(callback, function(job_ids){
      if(options.limit){
      ...
      _.each(job_ids, function(job_id){
        job_lookups.push(function(done){
          client.hgetall(kp+'_jobs:'+job_id, err(done, function(job){
            done(null, job);
          }));
        });
      });
