...
exports.add_receiver = function(options, callback){
  ...
  connection.query("INSERT INTO `receivers` ("+insert_args.cols_string+") VALUES ("+insert_args.vals_string+")", insert_args.vals, function(error, result){
...
exports.update_receiver = function(receiver_name, options, callback){
  ...
  connection.query("UPDATE `receivers` SET "+update_args.cols_string+" WHERE `name` = ?", update_args.vals, function(error, result){
    if(error){
      callback(error)
    } else {
      callback(null, result);
...
exports.remove_receiver = function(receiver_name, callback){
  exports.find_receiver_by_name(receiver_name, function(error, row){
    if(error){
      ...
      connection.query("DELETE FROM `receivers` WHERE `_id` = ?", [row._id], function(error, result){
        if(error) return callback(error);
        callback();
      });
    }
...
exports.remove_all = function(callback){
  ...
  _.each(tables, function(table){
    table_functions.push(function(done){
      connection.query("DELETE FROM `"+table+"`", [], function(error, result){
        if(error){
          done(error);
        } else {
          done();
...
exports.find_jobs_by_receiver_id = function(receiver_id, options, callback){
  ...
  connection.query(q, where_vars, function(err, job_rows){
...
exports.find_jobs_by_receiver_name = function(receiver_name, options, callback){
  if(receiver_name){
    ...
    connection.query(q, where_obj.vars, function(err, rows){
      if(err){
        callback(err);
      } else {
        ...
        _.each(rows, function(row){
          receiver_lookup_functions[row.receiver_id] = function(done){
            connection.query("SELECT `name` FROM `receivers` WHERE `_id` = ?", [row.receiver_id], function(err, receiver_rows){
              if(err){
                done(err);
              }
...
exports.create_job = function(options, callback){
  var insert_args = build_insert_args(options);
  connection.query("INSERT INTO `jobs` ("+insert_args.cols_string+") VALUES ("+insert_args.vals_string+")", insert_args.vals, function(err, result){
    if(err){
      callback(err);
    } else {
...
exports.get_job = function(id, callback){
  connection.query("SELECT * FROM `jobs` WHERE `_id`=?",[id], function(err, result){
    if(err){
      callback(err);
    } else {
      if(result.length == 1){
...
function find_receivers(options, callback, limit){
  ...
  connection.query(receiver_query, where_args.values, function(error, rows){
    if(error) return callback(error);
    callback(null, rows);
  });
};
